...
const C = {
  accent: "#6c63ff", teal: "#4ecdc4", coral: "#ff6b6b",
  amber: "#ffd166", mint: "#06d6a0", pink: "#f72585",
  sky: "#4cc9f0", olive: "#a8dadc",
}
const PALETTE = Object.values(C)
...
function deriveInsights(data) {
  ...
  const maxCity = Math.max(...cityEntries.map(([, v]) => v), 1)
  const radarData = cityEntries.map(([name, value]) => ({
    city: name, colleges: Math.round((value / maxCity) * 100),
  }))
...
function KPI({ label, value, sub, color = "var(--accent)" }) {
  return (
    <div className="card" style={{ position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: -18, right: -18, width: 72, height: 72, borderRadius: "50%", background: color + "18" }} />
      <div style={{ fontSize: "clamp(24px, 5vw, 34px)", fontWeight: 800, color, lineHeight: 1, marginBottom: 4 }}>{value}</div>
...
const TIP = (props) => (
  <Tooltip contentStyle={{ background: "#0e1624", border: "1px solid rgba(108,99,255,0.3)", borderRadius: 8, fontSize: 12, color: "#e2e8f0" }}
    cursor={{ fill: "rgba(108,99,255,0.08)" }} {...props} />
)
...
function HBar({ name, value, total, color, rank }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 12 }}>
        <span style={{ color: "var(--text)", fontWeight: 500 }}>
          <span style={{ color: "var(--muted)", marginRight: 6, fontSize: 11 }}>#{rank}</span>{name}
        </span>
        <span style={{ color: "var(--muted)" }}>{value} · {pct(value, total)}%</span>
      </div>
      <div style={{ height: 6, borderRadius: 99, background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${w}%`, borderRadius: 99, background: color, transition: "width 0.6s ease" }} />
      </div>
    </div>
  )
}
...
export default function Analytics() {
  ...
  const { cityData, streamData, radarData, matrixData, ownershipData, topStream, topCity, streams } = deriveInsights(data)
  const TABS = ["overview", "cities", "streams", "distribution"]

  // Responsive grid: 1 col on mobile, 2 on desktop
  const grid2 = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 340px), 1fr))", gap: 20 }
  ...
  return (
    <div style={{ padding: "24px 16px", maxWidth: 960, margin: "0 auto", width: "100%" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 150px), 1fr))", gap: 14, marginBottom: 28 }}>
        <KPI label="Total Colleges" value={data.total} sub="in database" color={C.accent} />
        <KPI label="Cities Covered" value={cityData.length} sub="across Maharashtra" color={C.teal} />
        <KPI label="Streams" value={streamData.length} sub="courses tracked" color={C.amber} />
        <KPI label="Top City" value={topCity.name} sub={`${topCity.value} colleges · ${pct(topCity.value, data.total)}%`} color={C.coral} />
        <KPI label="Top Stream" value={topStream.name} sub={`${topStream.value} programs`} color={C.mint} />
      </div>

      {/* Tab bar — scrollable on mobile */}
      <div style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: "1px solid rgba(255,255,255,0.08)", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
        {TABS.map(t => (
      ...
      {activeTab === "overview" && (
        <div style={grid2}>
          <div className="card">
            ...
            <ResponsiveContainer width="100%" height={210}>
              <PieChart>
                <Pie data={cityData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80}
                  label={({ name, pct: p }) => `${name} ${p}%`} labelLine={false}>
                  {cityData.map((_, i) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} />)}
                </Pie>
                <TIP formatter={(v, n) => [`${v} colleges`, n]} />
              </PieChart>
          ...
          <div className="card">
            ...
            <ResponsiveContainer width="100%" height={210}>
              <BarChart data={streamData} layout="vertical" margin={{ left: 8 }}>
                <XAxis type="number" tick={{ fill: "var(--muted)", fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis type="category" dataKey="name" tick={{ fill: "var(--muted)", fontSize: 10 }} axisLine={false} tickLine={false} width={70} />
                <TIP formatter={v => [`${v} colleges`]} />
                <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                  {streamData.map((_, i) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} />)}
                </Bar>
          ...
          <div className="card">
            <SectionTitle sub="Estimated ownership type split">Ownership Breakdown</SectionTitle>
            <OwnershipPanel data={ownershipData} />
            <p style={{ fontSize: 11, color: "var(--muted)", marginTop: 14, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 10 }}>
              ⚠ Ownership split is estimated.
            </p>
          </div>
        </div>
      )}
      ...
      {activeTab === "cities" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div className="card">
            <SectionTitle sub="College count and percentage share per city">City Leaderboard</SectionTitle>
            {cityData.map((c, i) => <HBar key={c.name} {...c} total={data.total} color={PALETTE[i % PALETTE.length]} rank={i + 1} />)}
          </div>
          <div className="card">
            <SectionTitle sub="Top 4 streams distributed across cities">Stream Mix by City</SectionTitle>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={matrixData} margin={{ left: 0 }}>
                ...
                {streams.map((s, i) => (
                  <Bar key={s} dataKey={s} stackId="a" fill={PALETTE[i % PALETTE.length]}
                    radius={i === streams.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]} />
                ))}
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="card" style={{ overflowX: "auto" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 360 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                  {["#", "Stream", "Colleges", "Share", "Bar"].map(h => (
                    <th key={h} style={{ textAlign: h === "Bar" || h === "Colleges" || h === "Share" ? "right" : "left", padding: "8px 10px", color: "var(--muted)", fontWeight: 500, fontSize: 11 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {streamData.map((s, i) => (
                  <tr key={s.name} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "9px 10px", textAlign: "right", color: "var(--text)" }}>{s.value}</td>
                    <td style={{ padding: "9px 10px", textAlign: "right", color: "var(--muted)" }}>{s.pct}%</td>
                    <td style={{ padding: "9px 10px", textAlign: "right", minWidth: 80 }}>
                      <div style={{ height: 5, borderRadius: 99, background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${s.pct}%`, background: PALETTE[i % PALETTE.length], borderRadius: 99 }} />
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {activeTab === "distribution" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div className="card">
            <SectionTitle sub="Simulated stream share across cities">Stream Coverage Across Cities</SectionTitle>
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={matrixData}>
                <defs>
                  {streams.map((s, i) => (
                ...
                <Legend wrapperStyle={{ fontSize: 12, color: "var(--muted)" }} />
                {streams.map((s, i) => (
                  <Area key={s} type="monotone" dataKey={s} stroke={PALETTE[i]} fill={`url(#g${i})`} strokeWidth={2} />
                ))}
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div style={grid2}>
            <div className="card">
              ...
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={ownershipData} dataKey="value" cx="50%" cy="50%" innerRadius={45} outerRadius={70}
          ...
          <div className="card" style={{ background: "#0a1628", border: "1px solid rgba(108,99,255,0.2)" }}>
            <SectionTitle>Data Quality Notes</SectionTitle>
