...
export default function EntranceExams() {
  ...
  const [nearbyLoad, setNearbyLoad]       = useState(false)
  const [nearbySearched, setNearbySearched] = useState(false)
  const [examPanelOpen, setExamPanelOpen] = useState(false) // mobile: toggle exam list

  useEffect(() => {
    setSelected(null); setDetails(null); setExamList([]); setNearby([]); setNearbySearched(false)
    setExamsLoading(true)
    fetch(`${API_BASE}/exams/${stream}`)
      .then(r => r.json())
      .then(data => { setExamList(data.exams || []); setExamsLoading(false) })
  ...
  async function loadExam(exam) {
    setSelected(exam); setDetails(null); setLoading(true); setActiveTab("syllabus")
    setNearby([]); setNearbySearched(false); setExamPanelOpen(false)
    try {
      const res = await fetch(`${API_BASE}/exam-details/${stream}/${encodeURIComponent(exam)}`)
      const data = await res.json()
      if (data.subjects) {
        data.subjects = data.subjects.map((s, i) => ({ ...s, color: s.color || FALLBACK_COLORS[i % FALLBACK_COLORS.length] }))
      }
      setDetails(data)
    } catch (e) {
      setDetails({ exam_name: exam, error: String(e), subjects: [], youtube_resources: [], top_coachings: [], useful_websites: [], preparation_tips: [] })
  ...
  async function loadNearby() {
    if (!selected) return
    setNearbyLoad(true); setNearby([]); setNearbySearched(true)
    try {
      const params = new URLSearchParams({ exam: selected, city: userCity })
      const res = await fetch(`${API_BASE}/nearby-coachings?${params}`)
      const data = await res.json()
  ...
  return (
    <div style={{ padding: "24px 16px", maxWidth: 960, margin: "0 auto", width: "100%" }}>
      <h1 style={{ fontSize: "clamp(20px, 5vw, 26px)", marginBottom: 6, fontFamily: "Syne, sans-serif" }}>
        Entrance Exam Guide
      </h1>
      <p style={{ color: "var(--muted)", marginBottom: 24, fontSize: 13 }}>
        Select a stream and exam — syllabus, resources, and coaching info fetched live from AI.
      </p>

      {/* Stream pills — scrollable */}
      ...
      {/* Mobile: show exam list as dropdown toggle */}
      {!selected && (
        <div style={{ marginBottom: 16 }}>
          <button onClick={() => setExamPanelOpen(v => !v)} style={{
            ...
            <span>Select an exam for {stream}</span>
            <span>{examPanelOpen ? "▲" : "▼"}</span>
          </button>
        </div>
      )}

      {/* Layout: side list + detail panel */}
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 200px) 1fr", gap: 20, alignItems: "start" }}
        className="exam-layout">

        {/* Exam list — hidden on mobile unless toggle open or exam selected */}
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }} className="exam-list">
          {examsLoading && <p style={{ fontSize: 12, color: "var(--muted)", padding: "8px 4px" }}>Loading exams…</p>}
          {!examsLoading && examList.length === 0 && (
            <p style={{ fontSize: 12, color: "var(--muted)", padding: "8px 4px" }}>No exams found for {stream}</p>
          )}
          ...
          {details && !loading && (
            <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div className="card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                  <div>
                    <h2 style={{ fontSize: "clamp(16px, 4vw, 20px)", marginBottom: 3, fontFamily: "Syne, sans-serif" }}>
                      {details.exam_name}
                    </h2>
                    {details.full_form && <p style={{ color: "var(--muted)", fontSize: 12 }}>{details.full_form}</p>}
                  </div>
                ...
                {details.eligibility  && <p style={{ marginTop: 10, fontSize: 12, color: "var(--muted)" }}><strong style={{ color: "var(--text)" }}>Eligibility: </strong>{details.eligibility}</p>}
                {details.exam_pattern && <p style={{ marginTop: 5, fontSize: 12, color: "var(--muted)" }}><strong style={{ color: "var(--text)" }}>Pattern: </strong>{details.exam_pattern}</p>}
              </div>

              {/* Tab bar — scrollable */}
              <div style={{ display: "flex", gap: 6, flexWrap: "nowrap", overflowX: "auto", WebkitOverflowScrolling: "touch", paddingBottom: 4 }}>
                {tabs.map(t => (
                  <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
                    padding: "6px 14px", borderRadius: 20, border: "1px solid", flexShrink: 0,
                    borderColor: activeTab === t.id ? "var(--accent)" : "var(--border)",
                    background: activeTab === t.id ? "rgba(108,99,255,0.15)" : "transparent",
                    color: activeTab === t.id ? "var(--accent)" : "var(--muted)",
                    cursor: "pointer", fontSize: 12, fontFamily: "inherit", transition: "all 0.15s",
              ...
              {activeTab === "syllabus" && (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 260px), 1fr))", gap: 14 }}>
                  {(details.subjects || []).map((subj, si) => {
                    ...
                    return (
                      <div key={si} className="card">
                        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: clr.text, background: clr.bg, padding: "5px 10px", borderRadius: 8, display: "inline-block" }}>
                          {subj.name}
                        </h3>
                        {(subj.topics || []).map((topic, ti) => (
                          <div key={ti} style={{ marginBottom: 10 }}>
                            ...
                            <div style={{ paddingLeft: 14, display: "flex", flexWrap: "wrap", gap: 4 }}>
                              {(topic.concepts || []).map((c, ci) => (
                                <span key={ci} style={{ fontSize: 11, padding: "2px 7px", borderRadius: 10, background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--muted)" }}>{c}</span>
                              ))}
                            </div>
                          </div>
                        ))}
                  ...
                  {(!details.subjects || details.subjects.length === 0) && <p style={{ color: "var(--muted)", fontSize: 13 }}>No syllabus data returned.</p>}
                </div>
              )}
              ...
              {activeTab === "youtube" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  {(details.youtube_resources || []).map((res, ri) => (
                    <div key={ri} className="card">
                      <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: "var(--accent)" }}>{res.subject}</h3>
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {(res.channels || []).map((ch, ci) => (
                          <a key={ci} href={ch.url} target="_blank" rel="noreferrer" style={{
                            display: "flex", justifyContent: "space-between", alignItems: "center",
                            padding: "10px 14px", background: "var(--surface2)", borderRadius: 8,
                            border: "1px solid var(--border)", textDecoration: "none", gap: 8,
                          }}>
                            <div style={{ minWidth: 0 }}>
                              <p style={{ fontSize: 13, fontWeight: 500, color: "var(--text)", marginBottom: 2 }}>{ch.channel}</p>
                              <p style={{ fontSize: 11, color: "var(--muted)" }}>{ch.playlist}{ch.subscribers ? ` · ${ch.subscribers}` : ""}</p>
                            </div>
                            <span style={{ fontSize: 12, color: "var(--accent)", whiteSpace: "nowrap", flexShrink: 0 }}>Watch →</span>
                          </a>
                        ))}
                      </div>
                    </div>
                  ))}
                  {(!details.youtube_resources || details.youtube_resources.length === 0) && <p style={{ color: "var(--muted)", fontSize: 13 }}>No YouTube resources returned.</p>}
                </div>
              )}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 280px), 1fr))", gap: 14 }}>
                  {(details.top_coachings || []).map((inst, ii) => (
                    <div key={ii} className="card" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      {inst.cities?.length > 0 && <p style={{ fontSize: 12, color: "var(--muted)" }}>📍 {inst.cities.join(", ")}</p>}
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {inst.fees_1yr && <span className="badge badge-purple">1-yr: ₹{inst.fees_1yr}</span>}
                        {inst.fees_2yr && <span className="badge badge-teal">2-yr: ₹{inst.fees_2yr}</span>}
                        {inst.crash_course_fees && <span className="badge badge-red">Crash: ₹{inst.crash_course_fees}</span>}
                      </div>
                      {inst.tenure_options?.length > 0 && <p style={{ fontSize: 12, color: "var(--muted)" }}><strong style={{ color: "var(--text)" }}>Programs: </strong>{inst.tenure_options.join(" · ")}</p>}
                      {(inst.highlights || []).length > 0 && (
                        <ul style={{ margin: 0, paddingLeft: 16 }}>
                          {inst.highlights.map((h, hi) => <li key={hi} style={{ fontSize: 12, color: "var(--muted)", marginBottom: 2 }}>{h}</li>)}
                        </ul>
                      )}
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {inst.website && <a href={inst.website} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "var(--accent)", padding: "4px 10px", border: "1px solid var(--accent)", borderRadius: 6, textDecoration: "none" }}>Website →</a>}
                        {inst.online_available && inst.online_url && <a href={inst.online_url} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "var(--muted)", padding: "4px 10px", border: "1px solid var(--border)", borderRadius: 6, textDecoration: "none" }}>Online →</a>}
                      </div>
                    </div>
                  ))}
                  {(!details.top_coachings || details.top_coachings.length === 0) && <p style={{ color: "var(--muted)", fontSize: 13 }}>No coaching data returned.</p>}
                </div>
              )}
              ...
              {activeTab === "nearby" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div className="card" style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    <input type="text" placeholder="Enter your city (e.g. Mumbai, Pune…)"
                      value={userCity} onChange={e => setUserCity(e.target.value)}
                      onKeyDown={e => e.key === "Enter" && loadNearby()}
                      style={{ flex: 1, minWidth: 200, padding: "9px 14px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 13, fontFamily: "inherit", outline: "none" }} />
                    <button onClick={loadNearby} disabled={nearbyLoad} style={{
                      padding: "9px 20px", borderRadius: 8, background: nearbyLoad ? "transparent" : "rgba(108,99,255,0.15)",
                      border: "1px solid var(--accent)", color: "var(--accent)", cursor: nearbyLoad ? "not-allowed" : "pointer",
                      fontSize: 13, fontFamily: "inherit", fontWeight: 600, whiteSpace: "nowrap", opacity: nearbyLoad ? 0.6 : 1,
                    }}>
                      {nearbyLoad ? "Searching…" : "Find Classes"}
                    </button>
                  </div>
                  ...
                  {nearbyLoad && (
                    <div className="card" style={{ textAlign: "center", padding: 32 }}>
                      <div className="spinner" style={{ margin: "0 auto 12px" }} />
                      <p style={{ color: "var(--muted)", fontSize: 13 }}>Finding coaching centers for {selected}…</p>
                    </div>
                  )}
                  {!nearbyLoad && !nearbySearched && (
                    <div className="card" style={{ textAlign: "center", padding: 32, color: "var(--muted)" }}>
                      <p style={{ fontSize: 22, marginBottom: 8 }}>📍</p>
                      <p style={{ fontSize: 13 }}>Enter your city above and click Find Classes</p>
                    </div>
                  )}
                  {!nearbyLoad && nearbySearched && nearby.length === 0 && (
                    <div className="card" style={{ textAlign: "center", padding: 32, color: "var(--muted)" }}>
                      <p style={{ fontSize: 13 }}>No coaching centers found. Try a different city.</p>
                    </div>
                  )}
                  {!nearbyLoad && nearby.length > 0 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      {nearby.map((inst, ii) => (
                        <div key={ii} className="card" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                            <div>
                              <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>{inst.name}</h3>
                              {inst.address && <p style={{ fontSize: 12, color: "var(--muted)" }}>📍 {inst.address}</p>}
                            </div>
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                              {inst.rating && <span className="badge badge-amber">★ {inst.rating}</span>}
                              {inst.distance_km && <span className="badge badge-teal">{inst.distance_km} km</span>}
                            </div>
                          </div>
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {inst.fees_per_year && <span className="badge badge-purple">₹{inst.fees_per_year} / yr</span>}
                            {inst.online_available && <span className="badge badge-teal">Online available</span>}
                          </div>
                          {inst.tenure_options?.length > 0 && <p style={{ fontSize: 12, color: "var(--muted)" }}><strong style={{ color: "var(--text)" }}>Programs: </strong>{inst.tenure_options.join(" · ")}</p>}
                          {(inst.highlights || []).length > 0 && (
                            <ul style={{ margin: 0, paddingLeft: 16 }}>
                          ...
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            {inst.website && <a href={inst.website} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "var(--accent)", padding: "4px 10px", border: "1px solid var(--accent)", borderRadius: 6, textDecoration: "none" }}>Website →</a>}
                            {inst.maps_query && <a href={`https://www.google.com/maps/search/${encodeURIComponent(inst.maps_query)}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "var(--muted)", padding: "4px 10px", border: "1px solid var(--border)", borderRadius: 6, textDecoration: "none" }}>Maps →</a>}
                            {inst.phone && <a href={`tel:${inst.phone}`} style={{ fontSize: 12, color: "var(--muted)", padding: "4px 10px", border: "1px solid var(--border)", borderRadius: 6, textDecoration: "none" }}>{inst.phone}</a>}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {activeTab === "websites" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {details.useful_websites.map((w, wi) => (
                          <a key={wi} href={w.url} target="_blank" rel="noreferrer" style={{
                            ...
                            <span style={{ fontSize: 12, color: "var(--accent)", whiteSpace: "nowrap", flexShrink: 0 }}>Open →</span>
                          </a>
                        ))}
                      </div>
                    </div>
                  ...
                  {(!details.useful_websites?.length && !details.online_courses?.length) && <p style={{ color: "var(--muted)", fontSize: 13 }}>No website data returned.</p>}
                </div>
              )}
              ...
              {activeTab === "tips" && (
                <div className="card" style={{ background: "rgba(108,99,255,0.06)", borderColor: "rgba(108,99,255,0.2)" }}>
                  <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 14 }}>Preparation Tips</h3>
                  {(details.preparation_tips || []).map((tip, ti) => (
                    <div key={ti} style={{ display: "flex", gap: 10, padding: "10px 0", borderBottom: ti < details.preparation_tips.length - 1 ? "1px solid var(--border)" : "none" }}>
                      <span style={{ fontSize: 13, color: "var(--accent)", fontWeight: 600, flexShrink: 0 }}>{String(ti + 1).padStart(2, "0")}</span>
                      <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.6 }}>{tip}</p>
                    </div>
                  ))}
                  {(!details.preparation_tips || details.preparation_tips.length === 0) && <p style={{ color: "var(--muted)", fontSize: 13 }}>No tips returned.</p>}
                </div>
              )}
      ...
      {/* Responsive: on mobile stack exam list above details */}
      <style>{`
        @media (max-width: 640px) {
          ...
          .exam-list {
            display: ${examPanelOpen || !selected ? "flex" : "none"} !important;
        ...
        @media (min-width: 641px) {
          .exam-layout { grid-template-columns: 200px 1fr !important; }
        }
      `}</style>
    </div>
  )
